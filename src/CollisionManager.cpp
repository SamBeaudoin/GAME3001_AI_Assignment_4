...
bool CollisionManager::AABBCheck(GameObject* object1, GameObject* object2)
{
	...
	const auto p1 = object1->getTransform()->position - glm::vec2(p1Width / 2, p1Height / 2);
	const auto p2 = object2->getTransform()->position - glm::vec2(p2Width / 2, p2Height / 2);
	const auto v1 = glm::vec2(object1->getRigidBody()->velocity.x, object1->getRigidBody()->velocity.y);
	const auto v2 = glm::vec2(object2->getRigidBody()->velocity.x, object2->getRigidBody()->velocity.y);
